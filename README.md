...
Use the project's included requirements.txt file to specify all packages needed: [aced_hmm.yml](./aced_hmm.yml)
```
...
We provide here an example workflow, together with an overview of the necessary commands, to optimize the model parameters such that true hospital-admissions counts are well fitted. To get started, we recommend running each command with the default parameters, which will optimize the parameters such that the retrospectively forecast hospital-admission numbers matches hospital-admission numbers observed by the state of MA 
