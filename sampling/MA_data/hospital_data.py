...
class HospitalData(object):
    ...
    def get_HHS_data(self, source_csv=None):
        ...
        if source_csv==None:
            ...
            return pd.read_csv(downloaded_csv) #tc.SFrame(downloaded_csv)

        else:
            hhs_data = pd.read_csv(source_csv) #tc.SFrame(source_csv)
            ...
            hhs_data_truncated = hhs_data.loc[:, columns_of_interest_hhs] #hhs_data.select_columns(columns_of_interest_hhs)
            # print(hhs_data_truncated)

            hhs_data_truncated['date'] = hhs_data_truncated.apply(lambda x: int(x['date'].replace("-", "")), axis=1) #hhs_data_truncated.apply(lambda x: int(x['date'].replace("-", "")) ) #for example: "MA" + "2020-01-01".replace("-","")
    ...
    def get_filtered_data(self):
        ...
        self.data['selected_row'] = self.data.apply(lambda x: 1 if (x['state']==self.us_state and (x['date']>=int(self._start_date) and x['date']<=int(self._end_date)) ) else 0, axis=1) #self.data.apply(lambda x: 1 if (x['state']==self.us_state and (x['date']>=int(self._start_date) and x['date']<=int(self._end_date)) ) else 0) #1 means row selected, 0 means row not selected for filtered data
        return self.data[self.data['selected_row'] == 1] #self.data.filter_by([1],'selected_row').sort(['date'],ascending=True)
    ...
    def get_InGeneralWard_counts(self):
        ...
        return (self.filtered_data.apply(lambda x: float('nan') if (None in [x['inpatient_beds_used_covid'],x['staffed_icu_adult_patients_confirmed_covid']]) \
            else x['inpatient_beds_used_covid']-x['staffed_icu_adult_patients_confirmed_covid'], axis=1)).to_numpy()
    ...
    def get_ICU_counts(self):
        ...
        return (self.filtered_data.apply(lambda x: float('nan') if (None in [x['staffed_icu_adult_patients_confirmed_covid']]) \
            else x['staffed_icu_adult_patients_confirmed_covid'], axis=1)).to_numpy()
    ...
    def get_pmf_num_per_timestep_InGeneralWard(self,errorFactor=1):
        ...
        previous_day_admission_adult_covid_confirmed = self.filtered_data.apply(lambda x: float('nan') if x['previous_day_admission_adult_covid_confirmed']==None else x['previous_day_admission_adult_covid_confirmed']/errorFactor, axis=1).to_numpy()
        ...
        pmf_SFrame = pd.DataFrame({'timestep':list(range(len(presenting_per_day))),'num_InGeneralWard':presenting_per_day}) #tc.SFrame({'timestep':list(range(len(presenting_per_day))),'num_InGeneralWard':presenting_per_day})
