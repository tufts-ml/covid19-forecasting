...
class HospitalData(object):
    ...
    def get_HHS_data(self):
        ...
        csv_name = 'reported_hospital_utilization_timeseries_20210227_1306.csv'
    ...
    def get_covidtracking_data(self):
        ...
        csv_name = 'covid_tracking_20210301.csv'
        today = datetime.now().date()
    ...
    def join_HHS_covidtracking_data(self, hhs_data, ctrack_data):
        ...
        ctrack_data_truncated = ctrack_data.loc[:, columns_of_interest_ctrack] #ctrack_data_truncated = ctrack_data.select_columns(columns_of_interest_ctrack)
        ...
        hhs_data_truncated['state_and_date'] = hhs_data_truncated.apply(lambda x: x['state'] + x['date'].replace("-", ""), axis=1) #for example: "MA" + "2020-01-01".replace("-","")
        ctrack_data_truncated['state_and_date'] = ctrack_data_truncated.apply(lambda x: x['state'] + str(x['date']), axis=1) #for example: "MA" + str(20200101)
        ...
        return hhs_data_truncated.merge(ctrack_data_truncated, on=['state_and_date'], how='left')

    def load_csv_if_exists(self):
        if os.path.isfile(self.csv_filename):
            #load
            self.data = pd.read_csv(self.csv_filename) #self.data = tc.SFrame(self.csv_filename)
            ...
            self.data.to_csv(self.csv_filename)  #self.data.save(self.csv_filename, format='csv') 
    ...
    def get_filtered_data(self):
        ...
        return self.data[self.data['selected_row'] == 1].sort_values(['date'], ascending=True) #self.data.filter_by([1],'selected_row').sort(['date'],ascending=True)
