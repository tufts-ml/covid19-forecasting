...
'''
SUMMARY_STATISTICS_NAMES = ["n_discharged_InGeneralWard", "n_InGeneralWard", "n_OffVentInICU", "n_OnVentInICU", "n_TERMINAL"] #, "n_admitted_InGeneralWard", "n_admitted_OffVentInICU", "n_admitted_OnVentInICU", "n_discharged_InGeneralWard", "n_discharged_OffVentInICU", "n_discharged_OnVentInICU"]
...
class ABCSampler(object):
    ...
    def select_theta_init_from_name(self, params_init):
        ...
        if params_init == "true_health":
            states = self.config_dict['states']

            health = [np.array([1 - self.config_dict['proba_Recovering_given_%s' % (state)], self.config_dict['proba_Recovering_given_%s' % (state)]]) for state in states]
            die_after_declining = [np.array([1 - self.config_dict['proba_Die_after_Declining_%s' % (state)], self.config_dict['proba_Die_after_Declining_%s' % (state)]]) for state in states if state != "OnVentInICU"]
            
            declining = [np.random.dirichlet(params) for params in self.abc_prior['durations']['Declining']]
            recovering = [np.random.dirichlet(params) for params in self.abc_prior['durations']['Recovering']]

        theta_init = {'health': health, 'die_after_declining': die_after_declining, 'durations': {'Declining': declining, 'Recovering': recovering}}
    ...
    def simulate_dataset(self, theta):
        ...
        for i in range(self.num_simulations):
            ...
            T_x = run_simulation(np.random.randint(100), self.config_dict, states, state_name_to_id, next_state_map)
    ...
    def abc_mcmc(self, theta_init, num_iterations, scale):
        ...
        for n in range(num_iterations):
            ...
            for state_index in range(len(self.config_dict['states'])):
                # draw from proposal distribution
                theta_health_prime = deepcopy(theta['health'])
                p_prime_D = self.draw_proposal_distribution_categorical(theta_health_prime[state_index], scale)
                ...
                if self.accept(distance):
                    ...
                    for s, state in enumerate(self.config_dict['states']):
                        ...
                        if state != 'OnVentInICU':
                            log_prop_prime += self.calc_log_proba_proposal_distribution_categorical(theta['die_after_declining'][s], theta_prime['die_after_declining'][s], scale)
                            log_prop_prev  += self.calc_log_proba_proposal_distribution_categorical(theta_prime['die_after_declining'][s], theta['die_after_declining'][s], scale)
                        
                        log_prop_prime += self.calc_log_proba_proposal_distribution_truncatednormal(theta['durations']['Declining'][s]['lam'], theta_prime['durations']['Declining'][s]['lam'], 0.5, theta['durations']['Declining'][s]['lower'], theta['durations']['Declining'][s]['upper'])
                        log_prop_prev  += self.calc_log_proba_proposal_distribution_truncatednormal(theta_prime['durations']['Declining'][s]['lam'], theta['durations']['Declining'][s]['lam'], 0.5, theta['durations']['Declining'][s]['lower'], theta['durations']['Declining'][s]['upper'])
                        log_prop_prime += self.calc_log_proba_proposal_distribution_truncatednormal(theta['durations']['Recovering'][s]['lam'], theta_prime['durations']['Recovering'][s]['lam'], 0.5, theta['durations']['Declining'][s]['lower'], theta['durations']['Declining'][s]['upper'])
                        log_prop_prev  += self.calc_log_proba_proposal_distribution_truncatednormal(theta_prime['durations']['Recovering'][s]['lam'], theta['durations']['Recovering'][s]['lam'], 0.5, theta['durations']['Declining'][s]['lower'], theta['durations']['Declining'][s]['upper'])
                ...
                for health_state in ["Declining", "Recovering"]:
                    ...
                    # draw from proposal distribution
                    theta_durations_prime = deepcopy(theta['durations'][health_state])
                    lam_prime = self.draw_proposal_distribution_truncatednormal(theta_durations_prime[state_index]['lam'], 0.5, theta_durations_prime[state_index]['lower'], theta_durations_prime[state_index]['upper'])
                    ...
                    # draw from proposal distribution
                    theta_durations_prime = deepcopy(theta['durations'][health_state])
                    tau_prime = self.draw_proposal_distribution_normal(theta_durations_prime[state_index]['tau'], 0.1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hospital', default='refine')
    parser.add_argument('--config_template', default='toy_data_experiment/example_params') # template for config_file
    parser.add_argument('--input_template', default='toy_data_experiment/')
    parser.add_argument('--output_template', default='toy_data_experiment/abc')
    parser.add_argument('--random_seed', default=101, type=int) # currently not using it 
    parser.add_argument('--algorithm', default='abc')
    parser.add_argument('--num_iterations', default=1500, type=int) # number of sampling iterations 
    ...
    parser.add_argument('--annealing_constant', default=0.99982)
    ...
    parser.add_argument('--abc_prior_type', default='for_cdc_table')
    ...
    # Order: "", "is_InICU == 1", "is_InICU == 0", "is_OnVent == 1"
    # 50-64 age group
    T_y = np.array([0.10, 0.36, 0.22, 8.00, 14.00, 25.00, 2.00, 4.00, 7.00, 2.00, 6.00, 12.00])
